...
static void validate_subscriptions(const std::vector<SubscriptionPtrPair>& subscriptionPtrPairs, bool expect)
{
    for (const auto& subscriptionPtrPair : subscriptionPtrPairs) {
        if (subscriptionPtrPair.first != subscriptionPtrPair.second) {
            auto subscribed =
                subscriptionPtrPair.first->get_subscribers().count(subscriptionPtrPair.second) &&
                subscriptionPtrPair.second->get_subscriptions().count(subscriptionPtrPair.first);
            if (subscribed != expect) {
                FAIL();
            }
        } else
        if (subscriptionPtrPair.first->get_subscribers().count(subscriptionPtrPair.second) ||
            subscriptionPtrPair.first->get_subscriptions().count(subscriptionPtrPair.second)) {
            FAIL();
        }
    }
}
